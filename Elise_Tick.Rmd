...
#Below was me adding a row to make questing numeric 
...
#This is how you change colors of the outside of the box
...
#Should I delete the NAs- the dead? incomplete data for an individual like if they died on day 2
...
#My data is super skewed, not a normal distribution
#ask nick what I should be using as my random and fixed effects in my models

hist(TotaltimeQuesters$Questing.Height.Average)
#This is skewed and not normally distributed
#maybe i should transform with a log or something-ask nick
```




##ELISE CHECK THIS- TRANSFORM Residuals
```{r}
...
hist((TotaltimeQuesters$Total.Time.Spent.Questing)^(1/4)) #ask Nick about this one. 1/4 is not common, but gives best transformation to noramlity here...
...
#my t value is positive for both day 2 and 3 and its a significant p value, so this means significant
...
#Remember that we are assuming your data are normal here, but this isn't the case for total time spent questing. ttsq isn't a normal distribution so we will need to either transform or use new code (in next section)
...
##Repeatability with Infection Status in Binary SicTicz
```{r}
#without life stage
